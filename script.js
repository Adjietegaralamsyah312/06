document.addEventListener('DOMContentLoaded', () => {
    ...
    let files = [];
    let idCounter = 0;
    const MAX_FILE_SIZE = 500 * 1024 * 1024; // 500 MB, dibatasi agar browser tidak kehabisan memori
    ...
    function getFileIcon(type) {
        ...
        if (isOfficeType(type)) return 'OFFICE';
    ...
    function getFileType(file) {
        ...
        Object.assign(types, officeTypes);
        const detectedType = types[extension];
        if (detectedType) return detectedType;
        return file.type || 'application/octet-stream';
    }

    function getOutputExtension(blob, originalName) {
        const extensions = {
            'image/jpeg': 'jpg',
            'image/png': 'png',
            'image/webp': 'webp',
        ...
        if (extensions[blob.type]) return extensions[blob.type];
        const originalExtension = originalName.includes('.') ? originalName.split('.').pop() : 'bin';
    ...
    function getCompressionSettings(level) {
        const settings = {
            low: { imageQuality: 0.9, maxSizeMB: 2, maxWidthOrHeight: 2400, gzipLevel: 3, svgMode: 'safe' },
            medium: { imageQuality: 0.75, maxSizeMB: 1, maxWidthOrHeight: 1800, gzipLevel: 6, svgMode: 'standard' },
            high: { imageQuality: 0.55, maxSizeMB: 0.5, maxWidthOrHeight: 1200, gzipLevel: 9, svgMode: 'aggressive' }
        };
    ...
    function canRecompressWithLevel(type) {
        return type.startsWith('image/') || isTextType(type) || isOfficeType(type);
    }
        ...
        files.forEach((f) => {
            ...
            downloadBtn.addEventListener('click', (e) => {
                ...
                if (f.status === 'done' && f.compressedBlob) {
                    ...
                    const ext = getOutputExtension(f.compressedBlob, f.name);
    ...
    async function compressSvg(file, level) {
        try {
            ...
            if (settings.svgMode === 'aggressive') {
                optimizedSvg = optimizedSvg
            ...
            if (svgBlob.size >= file.size) {
                return { blob: file, size: file.size, type: 'unchanged' };
            }
            return { blob: svgBlob, size: svgBlob.size, type: 'svg' };
        } catch (err) {
            throw new Error(`Gagal kompres SVG: ${err.message}`);
        }
    }
    ...
    async function compressPdf(file) {
        ...
        try {
            ...
            if (errorMessage.includes('encrypt')) {
                throw new Error('PDF terenkripsi atau dilindungi kata sandi. Buka proteksinya lalu coba lagi.');
            }
            if (errorMessage.includes('xfa')) {
                throw new Error('PDF XFA belum didukung untuk kompresi di browser.');
            }
            throw new Error('PDF rusak atau menggunakan fitur yang belum didukung. File asli tidak diubah.');
        }
    }

    async function compressOfficePackage(file, level) {
        if (typeof fflate === 'undefined') {
            throw new Error('Library kompresi dokumen gagal dimuat. Periksa koneksi internet lalu muat ulang halaman.');
        }
        try {
            const fileBytes = new Uint8Array(await file.arrayBuffer());
            const entries = await new Promise((resolve, reject) => {
                fflate.unzip(fileBytes, (error, data) => {
                    error ? reject(error) : resolve(data);
                });
            });
            const compressionLevel = getCompressionSettings(level).gzipLevel;
            const compressedEntries = {};
            Object.entries(entries).forEach(([name, content]) => {
                compressedEntries[name] = content;
            });
            const zipped = await new Promise((resolve, reject) => {
                fflate.zip(compressedEntries, { level: compressionLevel }, (error, data) => {
                    error ? reject(error) : resolve(data);
                });
            });
            const blob = new Blob([zipped], { type: file.type || 'application/octet-stream' });
            if (blob.size >= file.size) {
                return { blob: file, size: file.size, type: 'unchanged' };
            }
            return { blob, size: blob.size, type: 'office' };
        } catch (err) {
            throw new Error(`Dokumen Office tidak dapat diproses: ${err.message}`);
        }
    }
    ...
    async function compressFile(fileEntry) {
        ...
        const runId = (fileEntry._runId || 0) + 1;
        fileEntry._runId = runId;
        fileEntry._processing = true;
        const initialStatus = fileEntry.status;

        const checkCancelled = () => {
            if (fileEntry._deleted || fileEntry.status !== initialStatus || fileEntry._runId !== runId) {
                return true;
            }
            return false;
        };
        ...
        try {
            ...
            if (type.startsWith('image/')) {
                result = type === 'image/svg+xml'
                    ? await compressSvg(file, level)
                    : await compressImage(file, type, level, outFormat);
            } else if (type === 'application/pdf') {
                result = await compressPdf(file);
            } else if (isTextType(type)) {
                result = await compressText(file, level);
            } else if (isOfficeType(type)) {
                result = await compressOfficePackage(file, level);
            } else {
            ...
            if (result.type === 'image') {
                ...
                fileEntry.compressionType = '✒️ SVG';
            } else if (result.type === 'office') {
                fileEntry.compressionType = '📁 Office';
            } else if (result.type === 'unchanged') {
                fileEntry.compressionType = type === 'application/pdf' || isOfficeType(type)
                    ? '✓ Sudah optimal'
                    : '↔️ Ukuran sama';
            } else {
            ...
            if (result.type === 'unsupported') {
                ...
                fileEntry.error = 'Format ini belum didukung. Gunakan DOCX, XLSX, PPTX, atau format OpenDocument.';
    ...
    function addFiles(fileListFromInput) {
        ...
        for (let file of fileListFromInput) {
            ...
            if (file.size > MAX_FILE_SIZE) {
                alert(`File "${file.name}" terlalu besar (maks 500 MB).`);
                continue;
            }
    ...
    async function downloadAllAsZip() {
        ...
        for (let f of doneFiles) {
            try {
                ...
                const ext = getOutputExtension(f.compressedBlob, f.name);
                const filename = makeUniqueFilename(`${baseName}_compressed.${ext}`, usedNames);
        ...
        const originalLabel = downloadAllBtn.lastChild;
        if (originalLabel) originalLabel.textContent = ' Membuat ZIP...';
        try {
            const zipped = await new Promise((resolve, reject) => {
                fflate.zip(zipData, (error, data) => error ? reject(error) : resolve(data));
            });
            ...
            alert('ZIP gagal dibuat. Silakan coba lagi.');
            console.error('Gagal membuat ZIP:', err);
        } finally {
            downloadAllBtn.disabled = false;
            downloadAllBtn.removeAttribute('aria-busy');
            if (originalLabel) originalLabel.textContent = ' Download Semua sebagai ZIP';
        }
    }
